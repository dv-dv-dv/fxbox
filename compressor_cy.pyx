...
cdef class Compressor:
    ...
    def __init__(self, threshold=cfg.threshold, ratio=cfg.ratio, knee_width=cfg.knee_width, pre_gain=cfg.pre_gain, post_gain=cfg.post_gain, attack=cfg.attack, release=cfg.release):
        ...
        cdef double db_to_log2_constant = libc.math.log10(2)*20
        ...
        self.attack =libc.math.exp(-1/(attack*cfg.samplerate))
        self.release =libc.math.exp(-1/(release*cfg.samplerate))
        ...
        self.l2log_param1 =libc.math.log2(self.k)
        
    cdef compress(self, np.ndarray[FTYPE_t, ndim=1] audio_in):
        cdef np.ndarray[FTYPE_t, ndim=1] gain_linear = self.calculate_gain(audio_in)
        cdef np.ndarray[FTYPE_t, ndim=1] audio_out = audio_in.astype(np.double)*gain_linear
        return audio_out.astype(np.int16)
    
    cdef calculate_gain(self, np.ndarray[ITYPE_t, ndim=1] audio_in):
        cdef np.ndarray[FTYPE_t, ndim=1] level_db = self.linear_to_log(audio_in)
        cdef np.ndarray[FTYPE_t, ndim=1] gain_computer_out = self.gain_computer(level_db)
        cdef np.ndarray[FTYPE_t, ndim=1] odb3 = level_db - gain_computer_out
        cdef np.ndarray[FTYPE_t, ndim=1] odb4 = self.level_detector(odb3)
        cdef np.ndarray[FTYPE_t, ndim=1] gain_in_db = self.post_gain - odb4
        cdef np.ndarray[FTYPE_t, ndim=1] gain_linear = self.log_to_linear(gain_in_db)
        return gain_linear
    # accepts an audio_level, a value that represents an audio signal's loudness in either db or abs.
    # outputs
    cdef level_detector(self, np.ndarray[FTYPE_t, ndim=1] audio_level):
        cdef np.ndarray[FTYPE_t, ndim=1] output = np.zeros(audio_level.shape[0], dtype=FTYPE)
    ...
    # accepts an audio_level, a value that represents an audio signal's loudness in either db or abs.
    cdef gain_computer(self, np.ndarray[FTYPE_t, ndim=1] audio_level):
        cdef np.ndarray[FTYPE_t, ndim=1] output = np.zeros(audio_level.shape[0], dtype=FTYPE)
        cdef int audio_level_length = audio_level.shape[0]
        for i in range(audio_level_length):
            if audio_level[i] < self.gc_param1:
                ...
                output[i] = audio_level[i] + self.gc_param3*(audio_level[i] - self.gc_param1)**2
            elif audio_level[i] > self.gc_param2:
                output[i] = audio_level[i]/self.ratio + self.gc_param4
        return output
    
    cdef linear_to_log(self, np.ndarray[ITYPE_t, ndim=1] linear_array):
        cdef int linear_array_length = linear_array.shape[0]
        cdef np.ndarray[FTYPE_t, ndim=1] db_array = np.zeros(linear_array.shape[0], dtype=FTYPE)
        for i in range(linear_array_length):
            if(linear_array[i]==0):
                db_array[i] = -999999999999.9
            else:
                db_array[i] =libc.math.log2(abs(linear_array[i])) - self.l2log_param1
            asdf = 2
        return db_array
    
    # accepts a db value and then converts it to linear
    cdef log_to_linear(self, np.ndarray[FTYPE_t, ndim=1] db_array):        
        cdef int db_array_array_length = db_array.shape[0]
        cdef np.ndarray[FTYPE_t, ndim=1] linear_array = np.zeros([db_array.shape[0]], dtype=FTYPE)
        for i in range(db_array_array_length):
            linear_array[i] = 2**db_array[i]
        return linear_array
    
cdef class Compressor2(Compressor):
    def compress(self, np.ndarray[ITYPE_t, ndim=2] audio_in):
        cdef np.ndarray[ITYPE_t, ndim=1] audio_in_mono = audio_in[:,0] + audio_in[:,1]
        cdef np.ndarray[FTYPE_t, ndim=1] gain_linear = self.calculate_gain(audio_in_mono)
